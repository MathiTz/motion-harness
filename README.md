...
- **Undo — exactly what it covers.** `/undo` restores the file-level effects of the last turn: every `write_file`, `replace_in_file` and `edit_files` call is snapshotted first, and `/undo` reverts them all (deleting files the turn created) in one step. Overwriting an existing file the agent hasn't read is refused, independent of undo. **`/undo` does not cover `run_command`** (or `run_script`/`run_python`) — an approved shell command that deletes, moves or overwrites a file (`rm`, `mv`, `git clean`, a script writing over a file) leaves no checkpoint, because there is no general way to snapshot the effect of an arbitrary command in advance (a `git commit`, a network request, or a package install cannot be "undone" by restoring bytes). This is a deliberate scope decision, not an oversight: implementing file-snapshotting specifically for `run_command`'s in-workspace writes is possible but changes behavior (extra copying before every risky command) and was left for a maintainer to decide is worth the cost, rather than built speculatively. Nor does it cover a CLI-delegate turn's edits (Claude Code / Codex, above) — their tools run outside this harness entirely. If you need to undo a `run_command` side effect, `git status`/`git diff`/`git checkout` on a version-controlled workspace remains the general fallback.
- **Resume — exactly what it restores.** `/resume <id>` reloads a saved session's prompt/response text as conversation history for a *new* turn; it never re-invokes any tool call from a past turn, so resuming a session after a side-effecting command already ran does not run that command again. It restores conversation context only — not in-flight tool state, not undo checkpoints (those live in-memory for the current process and do not survive a restart). If the harness process is killed while a turn is still running a tool call (a crash, `kill -9`, an OOM kill), that turn is not silently dropped from the transcript: a start marker is written before any tool runs, so a turn that never reached completion shows up on `/resume` as an explicit `[turn interrupted ...]` entry instead of vanishing, and you should check the workspace for whatever partial effect the interrupted tool call may have had (undo cannot help there either, per the point above, unless it was a plain file write).
